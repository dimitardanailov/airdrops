...
import {insertZksyncWallet} from '../helpers/insertWallets'
...
export async function insertDDanailovWallets() {
  const records = wallets.map(wallet => {
    return {
      wallet: mainWalletByPrivateKey(wallet.privateKey),
  ...
  for await (const record of records) {
    await insertZksyncWallet(record.wallet, record.privateKey, {
